...
solver = MyFrame(RefineNet, class_num, metric, learning_rate)
# 模型参数量打印及记录
...
for epoch in range(1, total_epoch + 1):
    ...
    for i in range(N):
        ...
        train_img = np.concatenate((cv2.resize(img[i], (SHAPE[0]//4,SHAPE[1]//4)), train_img), axis=1)
    ...
    for i in range(N):
        ...
        val_img = np.concatenate((cv2.resize(img[i], (SHAPE[0]//4,SHAPE[1]//4)), val_img), axis=1)[:,:,::-1]
